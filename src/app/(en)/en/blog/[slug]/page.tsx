...
import ArticleToc, { type TocEntry } from '../../../../(he)/v2/components/ArticleToc'
import ShareButton from '../../../../(he)/v2/components/ShareButton'
import ActionButton from '../../../../(he)/v2/components/ActionButton'
import RelatedPosts from '../../../../(he)/v2/components/RelatedPosts'
import ContactForm from '../../../../(he)/v2/components/ContactForm'
import Footer from '../../../../(he)/v2/components/Footer'
import type { BlogCard } from '../../../../(he)/v2/components/BlogIndex'
import { ChevronPrevIcon } from '../../../../(he)/v2/components/icons'
...
import styles from '../../../../(he)/v2/components/Article.module.css'
...
const toCard = (post: AdminPost, defaultAuthor: string): BlogCard => ({
  slug: post.slug,
  title: post.title,
...
export default async function EnArticlePage({ params }: Params) {
  ...
  return (
    <>
        ...
        <article className={styles.article}>
          <div className={styles.inner}>
            <nav className={styles.crumbs} aria-label={blog.crumbsLabel}>
              <Link href="/en" className={styles.crumbLink}>
                {blog.crumbHome}
              </Link>
              <ChevronPrevIcon className={styles.crumbChevron} />
              <Link href="/en/blog" className={styles.crumbLink}>
                {blog.title}
              </Link>
              <ChevronPrevIcon className={styles.crumbChevron} />
      ...
      <Footer locale="en">
        <ContactForm variant="footer" source={`en-blog-${slug}`} />
      </Footer>
    </>
  )
}
