...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const raw = getAuthorById(id)
  if (!raw) return {}
  const author = applyAuthorEn(raw)
  const canonical = `/en/blog/author/${author.id}`
  const title = `${author.name} – ${author.role} | ${SITE_NAME}`
  return {
    title,
    description: author.bio || `${author.name}, ${author.role} at ${SITE_NAME}.`,
    alternates: {
      canonical,
      languages: {
        'he-IL': `/blog/author/${author.id}`,
        en: canonical,
        'x-default': `/blog/author/${author.id}`,
      },
    },
    openGraph: {
      type: 'profile',
...
export default async function EnAuthorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const authorUrl = `${SITE_URL}/en/blog/author/${author.id}`
    ...
    <div className="relative flex flex-col min-h-screen bg-white">
      ...
      <BreadcrumbSchema
        items={[
          ...
          { label: author.name, href: `/en/blog/author/${author.id}` },
        ]}
      />
      ...
      <div className="flex min-h-0 w-full flex-1 flex-col">
        <main id="main-content" className="relative z-20 flex-1 bg-white shadow-[0_20px_50px_rgba(0,0,0,0.1)]">
          <div className="max-w-4xl mx-auto px-6 pt-8 pb-2" dir="ltr">
            <Breadcrumb items={[{ label: 'Blog', href: '/en/blog' }, { label: author.name }]}
              homeLabel="Home"
              homeHref="/en" />
          </div>

          {author.bio && (
            <div className="max-w-3xl mx-auto px-6 pt-4 pb-8" dir="ltr">
          ...
          <div className="max-w-7xl mx-auto px-6 pt-2 pb-16" dir="ltr">
            <h2 className="text-[#111827] font-bold text-xl md:text-2xl mb-6">Articles by {author.name}</h2>
            {posts.length > 0 ? (
              ...
              <p className="text-[#9ca3af]">No articles from this author yet.</p>
            )}
          </div>

        </main>
        <Footer locale="en">
          <ContactForm variant="footer" source={`en-author-${author.id}`} />
        </Footer>
      </div>
    </div>
  )
