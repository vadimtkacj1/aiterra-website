...
type NavItem = NavLink & { children?: NavLink[]; overviewLabel?: string; flag?: Flag }

function NavFlag({ flag }: { flag?: Flag }) {
  if (!flag) return null
  const Icon = flag === 'il' ? FlagIlIcon : FlagUsIcon
  return <Icon className={styles.navFlag} />
}
...
export default function Header() {
  ...
  const servicesHref = header.nav.find((item) => item.id === 'services')?.href ?? '/services'
  const portfolioHref = header.nav.find((item) => item.id === 'portfolio')?.href ?? '/projects'

  const submenus: Record<string, NavLink[]> = {
    services: servicesStack.items.map((item) => ({
      label: item.title,
      href: item.action.href.includes('/services/')
        ? item.action.href
        : `${servicesHref}#v2-service-${item.id}`,
    })),
            ...
            {navItems.map((item) =>
              item.children ? (
                ...
                <NavAnchor
                  key={item.href}
                  href={item.href}
                  className={item.flag ? `${styles.navLink} ${styles.navFlagOnly}` : styles.navLink}
                  title={item.flag ? item.label : undefined}
                >
                  <NavFlag flag={item.flag} />
                  <span className={item.flag ? styles.srOnly : undefined}>{item.label}</span>
                </NavAnchor>
              ),
            )}
          </nav>
