    ...
    items: [
      {
        ...
        a: 'Fifteen live projects built by the same in-house team: three online stores (Neot Sade, ALOVA and a digital retail store), seven marketing and lead-generation sites across cosmetics, skincare and personal branding (Hofit Cosmetics, Maayan Vaknin, Karin Cohen, Olie 6, Alexandra Patsina, Sous Chef, Eli Ben Yitzhak), one real-estate marketing project, one digital-promotion programme for a mortgage advisory practice and three AI and operations platforms for clients in Canada (Zentax, Apprify, Tech Embassy). Each has its own page setting out the business objective, the systems it was connected to and what was actually built. All of them were written in code, the sites in React and Next.js and the platforms in ASP.NET and React Native, instead of being assembled from a template and a stack of plugins, which is why the same team can also maintain and extend them afterwards.',
      },
      {
        q: 'Are the projects in the portfolio still live?',
        a: 'Yes. All fifteen are running in production and serving real customers today; none of them are concepts, demos or unbuilt pitches. Several are also under ongoing maintenance and organic search work with us, so the team that built the site is the team that keeps it updated and ranking. When a client takes a project offline or moves it to another supplier, we remove it from the portfolio rather than leaving it there to pad the list. During a discovery call you can ask to see whichever project is closest to your own situation live on screen, including its load speed, its behaviour on mobile and the analytics behind it, so you are judging measured performance rather than a static image.',
      },
      {
